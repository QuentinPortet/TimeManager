defmodule Api.AccountsTest do
  ...
  describe "clocks" do
    ...
    test "create_clock/1 with valid data creates a clock" do
      valid_attrs = %{status: true, time: ~N[2022-10-25 08:49:00]}

      assert {:ok, %Clock{} = clock} = Accounts.create_clock(valid_attrs)
      assert clock.status == true
      assert clock.time == ~N[2022-10-25 08:49:00]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~N[2022-10-26 08:49:00]}

      assert {:ok, %Clock{} = clock} = Accounts.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~N[2022-10-26 08:49:00]
    end
  ...
  describe "workingtimes" do
    ...
    test "create_workingtime/1 with valid data creates a workingtime" do
      valid_attrs = %{end: ~N[2022-10-25 08:50:00], start: ~N[2022-10-25 08:50:00]}

      assert {:ok, %Workingtime{} = workingtime} = Accounts.create_workingtime(valid_attrs)
      assert workingtime.end == ~N[2022-10-25 08:50:00]
      assert workingtime.start == ~N[2022-10-25 08:50:00]
    end
    ...
    test "update_workingtime/2 with valid data updates the workingtime" do
      workingtime = workingtime_fixture()
      update_attrs = %{end: ~N[2022-10-26 08:50:00], start: ~N[2022-10-26 08:50:00]}

      assert {:ok, %Workingtime{} = workingtime} = Accounts.update_workingtime(workingtime, update_attrs)
      assert workingtime.end == ~N[2022-10-26 08:50:00]
      assert workingtime.start == ~N[2022-10-26 08:50:00]
    end
